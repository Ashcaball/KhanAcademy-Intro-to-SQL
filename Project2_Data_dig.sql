...
/* I wanted to see what objects in space had a volume larger than 10000 */

SELECT type_of_object, avg(volume) AS avg_volume
FROM solar_system_objects
GROUP BY Type_of_object
HAVING avg_volume > 10000;

/* I wanted to see what objects in space had a density below 1.5 */ 

SELECT type_of_object, avg(density) AS avg_density
FROM solar_system_objects
GROUP BY Type_of_object
HAVING avg_density > 1.5;

/* Here I classified objects as "small","medium", or "large" based on their radius */ 

SELECT MAX(mean_radius) AS Highest, MIN (mean_radius) AS Smallest, AVG(mean_radius) 
FROM solar_system_objects;

SELECT count(*),
CASE 
    WHEN mean_radius >= 400000 THEN 'Large'
    WHEN mean_radius >= 15000 THEN 'Medium'
    WHEN mean_radius <= 3000 THEN 'Small'
    ELSE 'tiny'
    END as 'Size'
FROM solar_system_objects
GROUP BY size;

/* I found out the average radius for each distinct shape */

SELECT avg(mean_radius) AS avg_shape_radius, shape
FROM solar_system_objects
GROUP BY shape
